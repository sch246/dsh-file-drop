...
/**
 * Invitation shown while a file drag is over the owning region. Decoration
 * only: `pointer-events: none` keeps drag targeting on the region below.
 *
 ...
 */
export function DropOverlay({ disabled, labels, container }: {
  disabled: boolean
  labels: DropOverlayLabels
  container: HTMLElement
}) {
  return createPortal(
    <div className="dsh-file-drop-mask" role="status">
      <div className="dsh-file-drop-wrap">
        <div className="dsh-file-drop-illustration" aria-hidden="true">
          {disabled ? <UploadDisabledIllustration /> : <UploadIllustration />}
        </div>
        <div className="dsh-file-drop-title">{labels.title}</div>
        {!disabled && labels.desc !== undefined && <div className="dsh-file-drop-desc">{labels.desc}</div>}
      </div>
    </div>,
    container,
  )
