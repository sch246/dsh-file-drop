...
const receiptName = 'dsh-file-drop.patch-state'
...
function readReceipt(path) {
  ...
  if (value.format !== 1 || typeof value.patchSha256 !== 'string'
    ...
    throw new Error('file-drop: malformed Host ownership receipt; preserve it for recovery')
  }
  return value
}
...
export function managePatch({ checkout, patchPath, mode = 'check' }) {
  if (!['check', 'apply', 'remove'].includes(mode)) throw new Error('file-drop: unknown Host patch operation')
  const hostRoot = realpathSync(checkout)
  if (realpathSync(git(hostRoot, ['rev-parse', '--show-toplevel'])) !== hostRoot) {
    throw new Error('file-drop: DSH_CHECKOUT must name the Git checkout root')
  }
  ...
  const inspect = () => {
    ...
    if (receipt !== undefined && (receipt.patchSha256 !== patchSha256 || receipt.hostRoot !== hostRoot)) {
      throw new Error('file-drop: receipt identifies another patch or Host; refusing ownership transfer')
    }
    const forward = accepts(hostRoot, patch, false)
    const reverse = accepts(hostRoot, patch, true)
    if (receipt === undefined) {
      if (!forward || reverse) throw new Error('file-drop: Host adapter is unowned or has drifted; refusing adoption')
    ...
    throw new Error('file-drop: owned Host adapter has drifted; preserve source and receipt for recovery')
  }
  if (mode === 'check') {
    const current = inspect()
    if (current.receipt !== undefined && current.receipt.status !== 'applied') {
      throw new Error(`file-drop: interrupted ${current.receipt.status} operation; resume it explicitly`)
    }
  ...
  try {
    const current = inspect()
    if (mode === 'apply') {
      if (current.receipt?.status === 'removing') throw new Error('file-drop: finish the interrupted removal first')
      ...
      if (!accepts(hostRoot, patch, true)) throw new Error('file-drop: applied adapter failed reverse verification')
    ...
    if (!accepts(hostRoot, patch, false)) throw new Error('file-drop: removed adapter failed forward verification')
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    if (checkout === undefined || checkout === '') throw new Error('file-drop: set DSH_CHECKOUT explicitly')
    const root = resolve(dirname(fileURLToPath(import.meta.url)), '..')
    const mode = (process.argv[2] ?? '--check').replace(/^--/, '')
    console.log(`file-drop: Host adapter ${managePatch({ checkout, patchPath: resolve(root, 'patches/deepseek-harness.patch'), mode })}`)
  } catch (error) {
